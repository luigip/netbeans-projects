package animatorexample;

/*File Animate01.java
Copyright 2001, R.G.Baldwin

This program displays several animated colored spherical creatures swimming 
around in an aquarium.  Each creature maintains generally the same course
with until it collides with another creature or with a wall.  However, 
each creature has the ability to occasionally make random changes in its course.
...
public class Animate01 extends Frame
    ...
    Animate01() {
        ...
        //Base the Frame size on the size of the background image.
        //These getter methods return -1 if the size is not yet known.
        ...
        //known yet.  Do the following just in case, wait until size is known
    ...
    public void run() {
        ...
        for (int cnt = 0; cnt < 15; cnt++) {
            Point position = spriteManager.getEmptyPosition(new Dimension(
                    gifImages[0].getWidth(this), gifImages[0].getHeight(this)));
            spriteManager.addSprite(makeSprite(position, cnt % 6));
        }//end for loop

        //Loop, sleep, and update sprite positions once each 83 milliseconds
    ...
    //Overridden graphics update method on the Frame
    ...
    public void paint(Graphics g) {
        //Nothing required here.  All drawing is done in the update method above.
...
class SpriteManager extends Vector {
    ...
    public void update() {
        ...
        for (int cnt = 0; cnt < size();
            ...
            //Test for collision. Positive result indicates a collision
            int hitIndex =
    ...
    private int testForCollision(Sprite testSprite) {
        ...
        for (int cnt = 0; cnt < size(); cnt++) {
            sprite = (Sprite) elementAt(cnt);
            if (sprite == testSprite) //don't check self
            {
                continue;
            }
            //Invoke testCollision method of Sprite class to perform the actual test.
            if (testSprite.testCollision(sprite)) //Return index of colliding sprite
            {
                return cnt;
            }
        }//end for loop
    ...
    public void drawScene(Graphics g) {
        //Draw the background and erase sprites from graphics area
        //Disable the following statement for an interesting effect.
        backgroundImage.drawBackgroundImage(g);

        //Iterate through sprites, drawingeach sprite
        for (int cnt = 0; cnt < size(); cnt++) {
...
class Sprite {
    ...
    public Sprite(Component component, Image image, Point position, Point motionVector) {
        //Seed a random number generator for this sprite with the sprite position.
        ...
        //Compute edges of usable graphics area in the Frame.
        ...
        bounds = new Rectangle(0 + leftBorder, 0 + topBanner, component.getSize().width
                - (leftBorder + rightBorder), component.getSize().height - (topBanner + bottomBorder));
    }//end constructor
    //---------------------------------//
    ...
    public void updatePosition() {
        ...
        //Insert random behavior.  During each update, a sprite has about
        // one chance in 10 of making a random change to its 
        // motionVector.  When a change occurs, the motionVector
        // coordinate values are forced to fall between -7 and 7.  This 
        // puts a cap on the maximum speed for a sprite.
